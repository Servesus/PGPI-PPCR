 ...
 */
function catch_foodmania_promo_headline_bg_css() {
	$enable_section = get_theme_mod( 'catch_foodmania_promotion_headline_visibility', 'homepage' );

	if ( ! catch_foodmania_check_section( $enable_section ) ) {
		// Bail if promotion_headline content is not enabled
		return;
	}

	$type = get_theme_mod( 'catch_foodmania_promotion_headline_type', 'page' );
	$css = '';

	if ( 'page' === $type || 'post' === $type || 'category' === $type ) {
		if ( 'page' === $type && $id = get_theme_mod( 'catch_foodmania_promotion_headline_page' ) ) {
			$id = absint( $id );
		} elseif ( 'post' === $type && $id = get_theme_mod( 'catch_foodmania_promotion_headline_post' ) ) {
			$id = absint( $id );
		} elseif ( 'category' === $type && $cat = get_theme_mod( 'catch_foodmania_promotion_headline_category' ) ) {
			$args['cat']            = absint( $cat );
			$args['posts_per_page'] = 1;
			$post_args = array( $args );
		...
		if ( has_post_thumbnail( $id ) ) {
			$css = '
				#promotion-headline {
					background: url(\'' . get_the_post_thumbnail_url( $id, 'catch-foodmania-slider' ) . '\');
		...
		$image = get_theme_mod( 'catch_foodmania_promotion_headline_image' );
...
add_action( 'wp_enqueue_scripts', 'catch_foodmania_promo_headline_bg_css', 11 );
