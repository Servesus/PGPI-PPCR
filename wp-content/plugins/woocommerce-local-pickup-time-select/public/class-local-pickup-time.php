...
class Local_Pickup_Time {
	...
	public function build_pickup_time_options() {
		...
		// Translateble days.
		__( 'Lunes', 'woocommerce-local-pickup-time' );
		__( 'Martes', 'woocommerce-local-pickup-time' );
		__( 'Miercoles', 'woocommerce-local-pickup-time' );
		__( 'Jueves', 'woocommerce-local-pickup-time' );
		__( 'Viernes', 'woocommerce-local-pickup-time' );
		__( 'Sabado', 'woocommerce-local-pickup-time' );
		__( 'Domingo', 'woocommerce-local-pickup-time' );
		...
		$pickup_options[''] = __( 'Seleccione la hora', 'woocommerce-local-pickup-time' );
	...
	public function time_select( $checkout ) {
		echo '<div id="local-pickup-time-select"><h2>' . __( 'Hora de envío', 'woocommerce-local-pickup-time' ) . '</h2>';
		...
		woocommerce_form_field(
			...
			array(
				...
				'label'    => __( 'Seleccione la hora de envío', 'woocommerce-local-pickup-time' ),
				'required' => true,
		...
		echo '<div id="local-pickup-time-select-note"><p>' . __( 'Nota: Si el horario de envío escogido entra dentro del horario en el que el comercio se encuentra cerrado, su pedido será cancelado y el importe del mismo devuelto.', 'woocommerce-local-pickup-time' ) . '</p>';
		echo '</div>';
	...
	public function field_process() {
		...
		if ( ! $_POST['local_pickup_time_select'] ) {
			wc_add_notice( __( 'Seleccione la hora de envío.', 'woocommerce-local-pickup-time' ), 'error' );
		}
